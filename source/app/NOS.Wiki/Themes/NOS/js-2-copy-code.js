var copyCode =
{
	...
	{
		var link = $("<a>")
			.text(copyCode.copyCodeText)
			.attr("title", copyCode.copyCodeTitle)
			.addClass("copy-code")
			.click(function() {
				copyCode.copyToClipboard(this);
			});
			
		link.insertBefore("#PageContentDiv pre");
	},
	copyToClipboard: function(link)
	{
		...
		var id = copyCode.rand++;
		
		var flash = $("<embed>")
				.attr("class", id)
				.attr("src", "Themes/NOS/clipboard.swf")
				.attr("flashvars", "highlighterId=" + id)
				.attr("menu", false)
	...
	{
		element.animate({ backgroundColor: "#E1DF9E" }, 100);
		element.animate({ backgroundColor: "#F6F6F6" }, 400);
	}
};
...
var SyntaxHighlighter =
{
	toolbar: {
		executeCommand: function(sender, event, highlighterId, commandName, args)
		{
			if (args !== "get")
			{
				$("pre#" + highlighterId).remove();
				$("embed." + highlighterId).remove();
			}
			return $("pre#" + highlighterId).text();$("<pre>")
		}
	}
}
