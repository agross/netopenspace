var copyCode =
{
	...
	init: function()
	{
		$("#PageContentDiv pre").each(function()
		{
			$(this).before(copyCode.createLink(this));
		});
	},
	createLink: function(pre)
	{
		if (!pre)
		{
			return $("<div>");
		...
		$(pre).attr("id", copyCode.rand++);

		return $("<div>")
			.addClass("copy-code")
			.flash(
				{
					src: "Themes/NOS/clipboard.swf",
					width: 77,
					height: 20,
					menu: "false",
					scale: "noScale",
	...
	flash: function(id)
	{
		$("#" + id).animate({ backgroundColor: "#E1DF9E" }, 100);
		$("#" + id).animate({ backgroundColor: "#F6F6F6" }, 400);
	}
};
